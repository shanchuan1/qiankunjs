...
const DEFAULT_OPTIONS = { capture: false, once: false, passive: false };

// 移除cacheListener
const removeCacheListener = (listenerMap, type, rawListener, rawOptions) => {
  // 处理 options，确保它是一个对象
  let options =
    typeof rawOptions === "object" ? rawOptions : { capture: !!rawOptions };
...
const addCacheListener = (listenerMap, type, rawListener, rawOptions) => {
  // 处理 options，确保它是一个对象
  let options =
    typeof rawOptions === "object" ? rawOptions : { capture: !!rawOptions };
  // 如果 options 为 null，使用默认值
...
export default function patch(global) {
  ...
  global.addEventListener = (type, rawListener, rawOptions) => {
    const addListener = addCacheListener(
      listenerMap,
      type,
      rawListener,
      rawOptions
  ...
  global.removeEventListener = (type, rawListener, rawOptions) => {
    const { listener, options } = removeCacheListener(
      listenerMap,
      type,
      rawListener,
      rawOptions
